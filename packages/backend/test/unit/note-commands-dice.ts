...
describe('note-commands dice', () => {
	test('/dice2d6 は合計値を MFM で埋め込む', () => {
		...
		const m = result.match(/^\/dice2d6 = \(\$\[bg\.color=0000ff (\d+)\]\)$/);
	...
	test('/dice3b6 はバラ目と合計を MFM で埋め込む', () => {
		...
		const m = result.match(/^\/dice3b6 = \(\$\[bg\.color=0000ff (\d+), (\d+), (\d+) \((\d+)\)\]\)$/);
		assert.ok(m, `unexpected format: ${result}`);
	...
	test('/dd は 1〜100 の範囲に展開される', () => {
		const result = run('/dd');
		const m = result.match(/^\/dd = \(\$\[bg\.color=0000ff (\d+)\]\)$/);
		assert.ok(m, `unexpected format: ${result}`);
		const v = Number(m![1]);
	...
	test('複数行のダイスコマンドをそれぞれ独立して展開する', () => {
		...
		assert.match(lines[1], /^\/dice1d20 = \(\$\[bg\.color=0000ff \d+\]\)$/);
	});
	...
	test('展開のたびに remainingBudget を消費する', () => {
		...
		const lines = result.split('\n');
		assert.match(lines[0], /\$\[bg\.color=0000ff \d+\]/);
