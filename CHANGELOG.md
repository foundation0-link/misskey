...
### General
- Feat: 1つのノートに対して複数のリアクションを付けられるように (1ユーザーあたり最大16件)
	- リモートサーバーは1ユーザー1リアクションを前提としているため、連合へ配信されるのは最初に付けたリアクションのみです

### Client
- Enhance: 未ログイン時のトップページ (classic) からタイムラインのサンプル表示を削除
- Enhance: すでにリアクション済みのノートでもリアクションを追加できるように

### Server
- Enhance: `notes/reactions/delete` に `reaction` パラメータを追加し、取り消すリアクションを指定できるように (省略時は従来どおり付けているリアクションのうち1件を削除)
- Enhance: ノートに `myReactions` を追加し、自分が付けているリアクションをすべて取得できるように (`myReaction` は1件目を返す従来互換フィールドとして維持)
- Fix: 連合が無効 (`federation` が `none`) の場合に nodeinfo が応答し、サーバー情報が外部へ公開されていた問題を修正
- Fix: 連合が無効 (`federation` が `none`) の場合でも配送ジョブがキューに投入されていた問題を修正
